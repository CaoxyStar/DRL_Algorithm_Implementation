...
from AC import AC_Agent, ReplayBuffer
...
def train():
    ...
    # Agent
    agent = AC_Agent(obs_space_dims, hidden_size, action_space_dims, actor_lr, critic_lr, gamma)
    ...
    for episode in range(total_num_episodes):
        ...
        if (episode + 1) % 200 == 0:
            torch.save(agent.actor.state_dict(), f"AC_Inverted_Pendulum_{episode+1}_episode.pth")
    wrapped_env.close()
...
def test():
    ...
    agent = AC_Agent(obs_space_dims, hidden_size, action_space_dims)
    agent.actor.load_state_dict(torch.load("weights/AC_Inverted_Pendulum.pth", weights_only=True))
